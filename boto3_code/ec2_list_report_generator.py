...
and send it in a S3 bucket created . Result will be list
'''
...
CC_EMAIL = ['info@cloudspaceacademy.com', 'joseph.mbatchou.aus@gmail.com']
current_date = datetime.now().strftime("%Y-%m-%d")


# Fisrt Function: List all Ec2 Instances and didderents parameters
def list_ec2_instances():
    ...
    for reservation in response['Reservations']:
        for instance in reservation['Instances']:
            ...
            list_ec2_instances.append([instance_id, name, state, instance_type, key_name, monitoring, availability_zone, vpc_id, subnet_id, public_ip, launch_time])
...
def generate_csv_report(instances):
    ...
    try:
        with open(FILE_NAME, 'w', newline='') as csvfile:
            fieldnames = ['instance_id', 'name', 'state', 'instance_type', 'key_name', 'monitoring', 'availability_zone', 'vpc_id', 'subnet_id', 'public_ip', 'launch_time']
